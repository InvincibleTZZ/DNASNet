...
from genotypes import Genotype, SHD_PRIMITIVES, SHD_BACK_PRIMITIVES
...
class SHDIdentityOp(nn.Module):
    def forward(self, x):
        return x


class SHDZeroOp(nn.Module):
    def forward(self, x):
        return x.mul(0.0)


class SHDLinearOp(nn.Module):
    def __init__(self, C, node_factory):
        super().__init__()
        self.linear = nn.Linear(C, C, bias=False)
        self.norm = nn.LayerNorm(C)
        self.node = node_factory()

    def forward(self, x):
        return self.node(self.norm(self.linear(x)))


class SHDStem(nn.Module):
    def __init__(self, input_dim, C, node_factory):
        super().__init__()
        self.linear = nn.Linear(input_dim, C, bias=False)
        self.norm = nn.LayerNorm(C)
        self.node = node_factory()
...
class SHDPreprocess(nn.Module):
    def __init__(self, C_in, C_out, node_factory):
        super().__init__()
        if C_in == C_out:
            self.op = nn.Identity()
        else:
...
def _build_shd_op(name, C, node_factory):
    if name.endswith('_back'):
        name = name[:-5]
    if name == 'identity':
        return SHDIdentityOp()
    if name == 'zero':
        return SHDZeroOp()
    if name == 'linear':
        return SHDLinearOp(C, node_factory)
    raise ValueError(f'Unsupported SHD operation: {name}. Expected one of {SHD_PRIMITIVES}.')


class SHDCell(nn.Module):
    def __init__(self, genotype, C_prev_prev, C_prev, C, node_factory,
                 back_connection=False):
        super().__init__()
        self.back_connection = back_connection
        self.preprocess0 = SHDPreprocess(C_prev_prev, C, node_factory)
        self.preprocess1 = SHDPreprocess(C_prev, C, node_factory)
        op_names, indices = zip(*genotype.normal)
        back_begin_index = len(op_names)
        ...
        self._steps = len(self._indices_forward) // 2
        self._ops = nn.ModuleList(
            [_build_shd_op(name, C, node_factory) for name in op_names[:back_begin_index]]
        )
        self._ops_back = nn.ModuleList()
        if self.back_connection:
            self._ops_back = nn.ModuleList(
                [_build_shd_op(name, C, node_factory)
                 for name in op_names[back_begin_index:]]
            )
    ...
    def forward(self, s0, s1):
        ...
        states = [s0, s1]
        for i in range(self._steps):
            h1 = self._ops[2 * i](states[self._indices_forward[2 * i]])
            h2 = self._ops[2 * i + 1](states[self._indices_forward[2 * i + 1]])
            s = h1 + h2
            if self.back_connection and i > 0 and (i - 1) < len(self._ops_back):
                target_idx = self._indices_backward[i - 1]
                states[target_idx] = states[target_idx] + self._ops_back[i - 1](s)
            states.append(s)
        return torch.cat([states[i] for i in self._concat], dim=-1)


def _format_shd_input(inputs):
    if inputs.dim() == 5:
        if inputs.size(2) == 1:
    ...
    if inputs.dim() != 3:
        raise ValueError(f'NetworkSHD expects [B, T, 700], got {tuple(inputs.shape)}')
    if inputs.size(-1) != 700:
        if inputs.size(1) == 700:
            inputs = inputs.transpose(1, 2)
        elif inputs.size(-1) != 700:
            raise ValueError(f'NetworkSHD expects 700 input channels, got {tuple(inputs.shape)}')
    return inputs


@register_model
class NetworkSHD(BaseModule):
    def __init__(self,
                 C,
                 num_classes,
                 layers,
        ...
        super(NetworkSHD, self).__init__(
            step=step,
        ...
        if genotype is None:
            from genotypes import shd_stdp
            genotype = shd_stdp
        if isinstance(node_type, str):
            node_cls = eval(node_type)
        else:
            node_cls = node_type
        self.node_factory = partial(node_cls, **kwargs)
        self.back_connection = kwargs.get('back_connection', False)
        self._layers = layers
        self.drop_path_prob = 0
        self.stem = SHDStem(input_dim, C, self.node_factory)

        C_prev_prev, C_prev, C_curr = C, C, C
        self.cells = nn.ModuleList()
        for _ in range(layers):
            cell = SHDCell(
                genotype, C_prev_prev, C_prev, C_curr, self.node_factory,
                back_connection=self.back_connection)
    ...
    def forward(self, inputs):
        inputs = _format_shd_input(inputs.float())
        self.reset()
        outputs = []
        for t in range(inputs.size(1)):
            x = self.stem(inputs[:, t, :])
            s0 = s1 = x
            for cell in self.cells:
                s0, s1 = s1, cell(s0, s1)
            outputs.append(self.classifier(s1))
